...
const int LED_PREPARING  = 4;   // preparing_cup (แดง)
const int LED_TOPPINGS   = 5;   // adding_toppings (เหลือง)
const int LED_ICE        = 18;  // adding_ice (น้ำเงิน)
const int LED_BREWING    = 19;  // brewing_drink (เขียว)
const int LED_COMPLETED  = 21;  // completed (ขาว)
const int SENSOR_PIN     = 34;  // สำหรับอนาคต (input only)
...
// ====== (Optional) เปิด/ปิด Debug redirect ======
const bool DEBUG_REDIRECT = true;
...
// เพิ่มตัวช่วยพิมพ์ Location ตอนเจอ 3xx
void debugRedirectIfAny(HTTPClient &http, int httpCode) {
  if (!DEBUG_REDIRECT) return;
  if (httpCode >= 300 && httpCode < 400) {
    // เก็บ header "Location" มาดู
    const char *heads[] = {"Location"};
    http.collectHeaders(heads, 1);
    String loc = http.header("Location");
    if (loc.length()) {
      Serial.println("[Redirect] Location: " + loc);
    } else {
      Serial.println("[Redirect] 3xx received but no Location header");
    }
  }
}
...
void pollForOrders() {
  ...
  if (httpCode == 200) {
    ...
    if (err) {
      Serial.println("JSON parse error on orders: " + String(err.c_str()));
    } else {
      if (doc["success"] && !doc["order"].isNull()) {
        // New order received
        String orderId = doc["order"]["id"].as<String>();
        String drinkName = doc["order"]["drinkName"].as<String>();
    ...
    debugRedirectIfAny(http, httpCode);
  }
...
void checkForCommands() {
  ...
  if (httpCode == 200) {
    ...
    DeserializationError err = deserializeJson(doc, payload);
    if (err) {
      Serial.println("JSON parse error on command: " + String(err.c_str()));
    } else if (doc["success"] && !doc["command"].isNull()) {
      // Execute command
    ...
    Serial.println("Error checking commands: " + String(httpCode));
    debugRedirectIfAny(http, httpCode);
  }
